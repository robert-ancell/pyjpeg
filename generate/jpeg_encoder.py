...
class ArithmeticDCTScanEncoder:
    ...
    def write_data_unit(self, data_unit, dc_table, ac_table):
        ...
        while k <= self.spectral_selection[1]:
            if k == 0:
                # FIXME: prev_dc_diff
                self.write_dc(zz_data_unit[k])
                k += 1
            else:
                run_length = 0
                while zz_data_unit[k] == 0 and k <= self.spectral_selection[1]:
                    run_length += 1
                if k + run_length > self.spectral_selection[1]:
                    ...
                    for _ in range(run_length):
                        self.decoder.write_bit(self.ac_non_zero[k - 1], 0)
                        k += 1
                    self.write_ac(zz_data_unit[k], k, self.kx[ac_table])
                    k += 1

    def write_dc(self, dc, prev_dc_diff):
        # FIXME: Classify prev_dc_diff
        c = 0

        if dc == 0:
            self.decoder.write_bit(self.dc_non_zero[c], 0)
            return
        self.decoder.write_bit(self.dc_non_zero[c], 1)
...
encoder = Encoder(
    [
        StartOfImage(),
        DefineQuantizationTables([QuantizationTable(8, 0, [1] * 64)]),
        StartOfFrame(8, 8, 8, 8, [FrameComponent(1, (1, 1), 0)]),
